...
public:
    ...
    int minCostClimbingStairI(vector<int>& cost) {
        int one_step_cost = cost[0];
        int two_steps_cost = cost[1];
...
int main() {
    ...
    vector<int(MinCostClimbingStairs::*)(vector<int>&)> funcs = {
        &MinCostClimbingStairs::minCostClimbingStairI,
    };

    for (auto func : funcs) {
        for (auto& test : test_cases) {
            vector<int> cost = test.first;
            int expected = test.second;

            int result = (Solution.*func)(cost);
