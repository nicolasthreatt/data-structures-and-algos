...
class HouseRobberI:
    ...
    # Algorithm Used: Dynamic Programming, One-Dimensional, Memo, Buttom-Up
    # Time Complexity: O(n)
    ...
    def robI(self, nums: List[int]) -> int:
        ...
        for i in range(1, len(nums)):
            two_houses_ago_amount = memo[i - 1] # House CAN be robbed
            one_house_ago_amount = memo[i] # House CANNOT be robbed
            num = nums[i]

            memo.append(max(two_houses_ago_amount + num, one_house_ago_amount))
    ...
    def robII(self, nums: List[int]) -> int:
        ...
        for num in nums:
            ...
            two_houses_ago_amount = max(one_house_ago_amount + num, two_houses_ago_amount) # House CAN be robbed
