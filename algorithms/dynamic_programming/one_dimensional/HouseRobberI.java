...
public class HouseRobberI {
    ...
    public int robI(int[] nums) {
        ...
        for (int i = 1; i < n; i++) {
            int two_houses_ago_amount = memo[i - 1];
            int one_house_ago_amount = memo[i];  // House CAN be robbed
            int curr_house_amount = nums[i];  // House CANNOT be robbed
            memo[i + 1] = Math.max(two_houses_ago_amount + curr_house_amount, one_house_ago_amount);
        }
    ...
    public int robII(int[] nums) {
        ...
        int two_houses_ago_amount = 0; // House CAN be robbed
        int one_house_ago_amount = nums[0]; // House CANNOT be robbed
