...
public:
    ...
    // Algorithm(s) Used: Dynammic Programming (1-D), Bottom-Up, Memoization
    // Time Complexity: O(n)
    // Space Complexity: O(n)
    int climbStairsIV(int n) {
        ...
        vector<int> memo = {1, 1};  // [one_step_away, two_steps_away]

        for (int i = 1; i < n; i += 1) {
            int one_step_moves = memo[i - 1];
            int two_step_moves = memo[i];
            memo.push_back(one_step_moves + two_step_moves);
        }
        return memo[n];
    }
