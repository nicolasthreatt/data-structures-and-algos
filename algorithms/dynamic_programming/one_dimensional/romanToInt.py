...
def romanToIntI(s: str) -> int:
    # Base Case: Empty String
    if not s:
        return 0

    # Start with the value of the first Roman numeral
    total = SYMBOLS_TO_VALUE[s[0]]
...
# Time Complexity: O(n)
# Space Complexity: O(1)
def romanToIntII(s: str) -> int:
    # Base Case: Empty String
    if not s:
        return 0

    # Start with the value of the first Roman numeral
    total = SYMBOLS_TO_VALUE.get(s[0])

    # Iterate string and check for valid subtractive pairs (IV, IX, XL, XC, CD, CM)
    for i in range(1, len(s)):
        curr = SYMBOLS_TO_VALUE.get(s[i])
        prev = SYMBOLS_TO_VALUE.get(s[i - 1])
...
if __name__ == "__main__":
    ...
    for func in [romanToIntI, romanToIntII]:
