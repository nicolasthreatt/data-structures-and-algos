...
class ClimbStairs:
    ...
    # Algorithms Used: Dynamic Programming (1-D), Top-Down, Memoization
    # Time Complexity: O(n)
    # Space Complexity: O(n)
    ...
    # Algorithms Used: Dynamic Programming (1-D), Bottom-Up, Memo
    # Time Complexity: O(n)
    # Space Complexity: O(n)
    def climbStairsIV(self, n: int) -> int:
        ...
        memo = [1, 1]  # [one_step_away, two_steps_away]
        for i in range(1, n):
            one_step_away = memo[i - 1]
            two_steps_away = memo[i]
            
            memo.append(one_step_away + two_steps_away)
