...
# Time Complexity: O(R * C * (4 ^ (R * C))), where
#   - R is the number of rows
#   - C is the number of columns
# Space Complexity: O(R * C * (4 ^ (R * C))), where
#   - R is the number of rows
#   - C is the number of columns
def longest_route(grid):
    ...
    def dfs(r, c):
        ...
        for dr, dc in adjacent_cells:
            if is_valid_cell(r + dr, c + dc) and grid[r + dr][c + dc] == grid[r][c]:
                area = max(area, 1 + dfs(r + dr, c + dc))

        # Remove the cell from the visited set (backtrack)
        # This ensures that the cell can be visited again in a different valid path
        visited.remove((r, c))
