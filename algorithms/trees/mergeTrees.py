...
# Time Complexity: O(n + m), where n = nodes in roo1 and m = nodes in root2
# Space Complexity: O(n + m)
def mergeTrees(root1: Optional[TreeNode], root2: Optional[TreeNode]) -> Optional[TreeNode]:
    # If both tree nodes are null, then theres nothing to merge (Base Case)
    if not root1 and not root2:
        return None
    
    # Now its known that atleast one of the nodes exist
    # Create a new node with the sum of the tree nodes
