...
# Time Complexity: O(root1 + root2) = O(n)
# Space Complexity: O(root1 + root2) = O(n)
def flipEquiv(root1: Optional[TreeNode], root2: Optional[TreeNode]) -> bool:
    ...
    # If two trees are the same then they are flipped equivalent
    option1 = (
        flipEquiv(root1.left, root2.left) and
        flipEquiv(root1.right, root2.right)
    )

    # If the two trees' opposite nodes are the same then they are flipped equivalent
    option2 = (
        flipEquiv(root1.left, root2.right) and
        flipEquiv(root1.right, root2.left)
    )

    # Return true of either flipped equivalent scenario
