...
SELECT
    ...
    SalaryRanks.name AS Employee,
    SalaryRanks.salary
FROM (
    SELECT
        *,
        RANK() OVER (PARTITION BY departmentId ORDER BY salary DESC) AS rankings
    FROM Employee
) SalaryRanks
LEFT JOIN Department
    ON SalaryRanks.departmentId = Department.id
WHERE SalaryRanks.rankings = 1;
