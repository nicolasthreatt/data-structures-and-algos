...
- client_id and driver_id are foreign keys to the users_id at the Users table.
...
unbanned users each day between 2013-10-01 and 2013-10-03 with at least one trip.
...
SELECT
    ...
    ROUND(
        SUM(CASE WHEN Trips.status != 'completed' THEN 1 ELSE 0 END) * 1.0 / COUNT(*), 2
    ) AS 'Cancellation Rate'
FROM Trips
JOIN Users BannedClients
    ON Trips.client_id = BannedClients.users_id
JOIN Users BannedDrives
    ON Trips.driver_id = BannedDrives.users_id
WHERE
    BannedClients.banned = 'No' AND
    BannedDrives.banned = 'No' AND
