...
SELECT
    ROUND(COUNT(b.first_event_date) * 1.0 / COUNT(DISTINCT a.player_id), 2) AS fraction
FROM Activity a
LEFT JOIN (
    SELECT
        player_id,
    ...
    FROM Activity
    GROUP BY player_id
) b ON a.player_id = b.player_id and DATEDIFF(DAY, b.first_event_date, a.event_date) = 1;
