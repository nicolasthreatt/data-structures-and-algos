...
class Twitter:
    ...
    def __init__(self):
        ...
        self.tweets = defaultdict(list)  # {userId: timeline[]}
        ...
        self.follows = defaultdict(set)  # {followerId: followeeIds[]}
    ...
    def getNewsFeed(self, userId: int) -> List[int]:
        ...
        """
        tweets = []   # Create a list to store the 10 latest tweets
        minHeap = []  # In Python, heapq always implements a min-heap

        # Add the user's tweets to the heap (noted in the problem description).
...
if __name__ == "__main__":
    ...
    assert twitter.getNewsFeed(1) == [9, 8, 7, 5]
    ...
    assert twitter2.getNewsFeed(1) == [15, 14, 13, 12, 11, 10, 9, 8, 7, 6]
    ...
    twitter3.follow(1, 2)
    twitter3.follow(1, 3)
    assert twitter3.getNewsFeed(1) == [301, 102, 201, 101]
    ...
    assert twitter4.getNewsFeed(1) == [42]